...
declare namespace FBInstant {
    ...
    function performHapticFeedbackAsync(): Promise<unknown>;
     ...
     * share which every game session launched from the invite will be able to access from FBInstant.getEntryPointData().
     * This data must be less than or equal to 1000 characters when stringified. The user may choose to cancel the action
     * and close the dialog, and the returned promise will resolve when the dialog is closed regardless if the user actually
     * invited people or not.
     *
     ...
     * @throws INVALID_OPERATION
     */
    function inviteAsync(payload: SharePayload): Promise<void>;
     ...
     * @param placementID The placement ID that's been setup in your Audience Network settings.
     * @returns A promise that resolves void, or rejects with a APIError if it couldn't be created.
     * @throws RATE_LIMITED
     * @throws CLIENT_UNSUPPORTED_OPERATION
     */
     ...
     * @param placementID The placement ID that's been setup in your Audience Network settings.
     * @returns A promise that resolves with void, or rejects with a APIError if it couldn't be created.
     * @throws CLIENT_UNSUPPORTED_OPERATION
     */
    function hideBannerAdAsync(placementID: string): Promise<void>;
