...
const AD_TYPE_TO_PLACEMENT_ID: { [key: number]: string } = {
    [AD_TYPE.INTERSTITIAL]: AdIDs[AdPlacements.INTERSTITIAL],
    [AD_TYPE.REWARDED]: AdIDs[AdPlacements.REWARDED],
    [AD_TYPE.BANNER]: AdIDs[AdPlacements.BANNER],
};
...
export class FBAdManager {
    ...
    public async preloadAdInstance(_type: AD_TYPE, _placementId?: string): Promise<FBInstant.AdInstance> {
        if (
            !ENGINE.platformSDK.isAdsSupported() ||
            this.isAdInstanceAvailable(_type) ||
            (!AD_TYPE_TO_PLACEMENT_ID[_type] && !_placementId)
        ) return;

        let retryCounter = 0;
        const amountOfRetries = this.getNumberOfRetriesFromType(_type);
        const retry = async (): Promise<FBInstant.AdInstance> => {
            ...
            try {
                ...
                if (!this._adInstances[_type]) {
                    throw new Error(FBAdManagerErrorCodes.FAILED_TO_FETCH_INSTANCE);
                }
                return this._adInstances[_type];
            } catch (err) {
                console.error(err);
                retryCounter++;
                return retry();
    ...
    public async showAd(_type: AD_TYPE): Promise<void> {
        if(!ENGINE.platformSDK.isAdsSupported()) return;
        ...
        const retryShow = async (): Promise<void> => {
            ...
            try {
                ...
                if (err && err.code === "USER_INPUT") {
                    this._adInstances[_type] = null;
                    throw new Error(FBAdManagerErrorCodes.USER_INPUT);
                }
                console.error(err);
                retryCounter++;
                return retryShow();
