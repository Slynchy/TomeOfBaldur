import {DisplayObject, InteractionEvent as PIXIInteractionEvent} from "pixi.js";
import {HelperFunctions} from "../../tsthree";
import isMobile from "is-mobile";
import {isTouchDevice} from "./isTouchDevice";

// HACK

let __PLAYING_SFX = false;
...
function buttonify_mobile<T extends DisplayObject>(
    _target: T | T[],
    _settings: IButtonifySettings
): void {
...
function buttonify_desktop<T extends DisplayObject>(
    _target: T | T[],
    _settings: IButtonifySettings
): void {
    const state: IButtonifyState = {
    ...
    const pointerUp = (ev: PIXIInteractionEvent) => {
        if(_settings.onPointerUp)
            _settings.onPointerUp(ev, state);
        if(
            _settings.skipPointerDownChecks ||
            (state.pointerDown && state.pointerOver)
        ) {
            _settings.onFire(ev, state);
        }
        state.pointerDown = false;
    ...
    const pointerDown = (ev: PIXIInteractionEvent) => {
        // if(!__PLAYING_SFX) {
        //     HelperFunctions.playSound_s(
        //         "click" + Math.floor(Math.random() * 4).toString()
        //     );
        //     __PLAYING_SFX = true;
        //     setTimeout(() => __PLAYING_SFX = false, 33);
        // }
        if(_settings.onPointerDown)
            _settings.onPointerDown(ev, state);
        state.pointerDown = true;
...
export function buttonify<T extends DisplayObject>(
    _target: T | T[],
    _settings: IButtonifySettings
): void {
