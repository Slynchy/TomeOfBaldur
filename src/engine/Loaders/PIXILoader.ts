...
import { ILoaderMiddleware, Loader as _PIXILoader, LoaderResource } from "pixi.js";
import { HelperFunctions } from "../HelperFunctions";
import { ENGINE_DEBUG_MODE } from "../Constants/Constants";

export class PIXILoader extends Loader<LoaderResource> {

    // @ts-ignore
    private loader: _PIXILoader;
    private readonly _cache: { [key: string]: LoaderResource };
            ...
            this.loader.load((loader: _PIXILoader, resources: { [key: string]: LoaderResource }): void => {
                for (const k in resources) {
                    if (resources[k]) {
                        ...
                        } else returnValue[k].success = true;
